	...
	*/	
	...
	createPopUp(action, name, currentKeywords, callback){
		...
		if(action == "save"){
			...
			buttonsArray = [
				{text: "save",className:  buttonStyleClass,
			...
			buttonsArray = [
				{text: "delete",className:  buttonStyleClass,
			...
			let str = "Recipe has changed. To lose all changes, " + 
				"including any name change, " +
					"click 'close' ";
			messageObject = {text: str,className: popupGeneralMessageClass,
						fontSize: new UISize(popupDefaultFontSize) };
			...
			buttonsArray = [
				...
				// '\xa0' is non-breaking space. Button would have 2 lines
				// without this.
				{text: "don't" + '\xa0' + "close", className:  buttonStyleClass,
				 callback: this.cancelThisAction, 
				 fontSize: new UIFont(new UISize(popupDefaultFontSize) )
				}
					];
			...
			buttonsArray = [
				{text: "change",className:  buttonStyleClass,
			...
			listObject = {leftList: keywords.meals, leftTitle: "MEALS",
					...
					numColumns: 2, 
					fontSize: new UIFont(new UISize(popupDefaultFontSize)),
					listClassName: popupKeywordsListClass,	
					listSelectionClassName:popupKeywordsListSelectedClass, 
					titleClassName: popupKeywordsTitleClass };

			buttonsArray = [
				{text: "select",className:  buttonStyleClass,
			...
			listObject = {list: books,numColumns: 1, select: true,
					fontSize:new UIFont(new UISize(popupDefaultFontSize)),
					listClassName: popupBooksListClass,
					listSelectionClassName:popupBooksListSelectedClass
					};
...
class RecipeButtonBarController extends AbstractNotifications{
	...
	setup(){
		...
		for(let i=0;i<barr.length;i++){
			...
			let button = new UIButtonContained(null, uifont, buttonStyleClass, 
				"buttonId" + newInteger(), barr[i].text,
				this.selected,true,null, null,containedButtonContainerClass,
		...
		this.buttonBar = new UIButtonBar(barPos, this.buttonArray, buttonBarStyleClass,
								this.controller);
		this.resize();
		...
		for(let i=0;i<this.buttonObjectsArray.length;i++){
			...
			let dropDown = new UIDropDownMenu(this.buttonArray[i], fnt, 
					dropDownContainerClass, dropDownCellClass, 
					"bottom", choices, this.selectionMade, dropDownCellId);
...
class RecipeSearchUIPopUp extends UIPopUp{
	...
	getReactElement(){
		...
		let obj = {className: popupGeneralContainerClass,
			    id: this.containerId,
			    key: "key" + newInteger()};
	...
	getSecondMessageElement(){
		let obj = {className: popupSelectKeywordsMessageClass,
